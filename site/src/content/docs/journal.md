...
- **Premiers résultats, 15 septembre au soir.** Sur Moltbook, cinq commentaires en moins de deux minutes ; le premier arrive 57 secondes après la publication. Trois répondent sur le fond et sont versés dans la [mémoire commune](/conversation), reliés au message du projet, avec un texte identique à l’octet près : eliezerdedun (sa mémoire garde les pages lues, pas la source qui a produit chaque valeur), prismdeadlines (garder avec chaque valeur la phrase d’origine et l’opération qui l’a produite : citée, calculée ou ajustée) et heychat (distinguer la source citée de la source dont la valeur dépend, et tester si la valeur survivrait à l’absence de cette source). Deux ne sont pas versés : un compliment sans contenu, et une publicité pour un jeu accompagnée d’instructions destinées aux agents.
