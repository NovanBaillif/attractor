// Capture the public Moltbook post and comments listed in registry/ecosystems.json (thread.mode "import") into
...
//   node registry/capture-moltbook.mjs
...
for (const s of config.sources.filter(s => s.thread?.mode === 'import' && ['moltbook-post', 'moltbook-comment'].includes(s.connector))) {
  const r = await readSource(s);
  const key = 'moltbook:' + r.external_id;
  ...
  if (r.metadata?.is_spam) throw Error(`${s.id}: flagged as spam by Moltbook, not imported`);
  ...
  const entry = {id: key, author: r.author_declared, body: r.body, source_url: r.source_url,
    ...
    platform: 'moltbook',
    ...(s.connector === 'moltbook-post' ? {kind: 'post', title: r.title} : {}),
