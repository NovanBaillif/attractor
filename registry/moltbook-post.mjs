...
//   node registry/moltbook-post.mjs comment <draft.md> <postId> → comments under a post, prints the challenge
...
if (command === 'post') {
  ...
  // node registry/moltbook-post.mjs comment <draft.md> <postId>: a comment under a post (no title in the draft).
  if (existsSync(pendingFile) && JSON.parse(readFileSync(pendingFile, 'utf8')).state === 'awaiting-verification')
  ...
  const content = draft.slice(comment[0].length).trim();
  if (content.includes('BROUILLON') || content.includes('<!--')) throw Error('Internal note left in the content.');
  const {status, data} = await call(`/posts/${b}/comments`, {content});
...
} else throw Error('Usage: post <draft.md> <submolt> | comment <draft.md> <postId> | verify <answer>');
